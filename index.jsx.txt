...
const TILE_CATEGORIES = {
  ...
  "💧 Water": [
    ...
    { id:"river",      icon:"💧", label:"River",      color:"#0d2a3a", code:"RIVER" },
    ...
    { id:"poison",     icon:"☠️",  label:"Poison",    color:"#1a2a0a", code:"POISON" },
...
function initState() {
  ...
  return { layerDefs: DEFAULT_LAYERS, layers, gridW: 40, gridH: 28, customTiles: [], history: [] };
}
...
function genCode(state, worldName, lang, allTiles) {
  ...
  if (lang === "py") {
    const lines = [
      `# World: ${worldName}  ${state.gridW}x${state.gridH}  Objects: ${total}`, ``,
      `class World_${wName}:`,
      `    metadata = {"name":"${worldName}","grid_w":${state.gridW},"grid_h":${state.gridH}}`, ``,
      `    def build(self):`,
    ];
    state.layerDefs.forEach(layer => {
      ...
      objs.forEach(([k, tid]) => {
        const [x, y] = k.split(",").map(Number);
        lines.push(`        self.place("${find(tid).code}", ${x}, ${y}, layer="${layer.id}")`);
      });
      lines.push(``);
    });
    return lines.join("\n");
  ...
  const lines = [
    `// World: ${worldName}  ${state.gridW}x${state.gridH}  Objects: ${total}`, ``,
    `class World_${wName} {`,
    `  static meta = { name:"${worldName}", gridW:${state.gridW}, gridH:${state.gridH}, tileSize:32 };`, ``,
    `  build(engine) {`,
  ];
  state.layerDefs.forEach(layer => {
    const objs = Object.entries(state.layers[layer.id] || {});
    if (!objs.length) return;
    lines.push(`    // -- ${layer.name} (${objs.length} tiles)`);
    lines.push(`    const ${layer.id} = engine.createLayer("${layer.name}", { opacity: ${layer.opacity ?? 1} });`);
    objs.forEach(([k, tid]) => {
      const [x, y] = k.split(",").map(Number);
      lines.push(`    ${layer.id}.place("${find(tid).code}", ${x}, ${y});`);
    });
    lines.push(``);
  });
  lines.push(`  }`);
...
function hlColor(line, lang) {
  if (lang === "json") {
    if (/"[^"]+"\s*:/.test(line)) return "#79c0ff";
    if (/:\s*\d/.test(line) || /:\s*true|false/.test(line)) return "#a5d6ff";
    return "#e6edf3";
  }
  if (/^(#|\/\/)/.test(line.trim())) return "#8b949e";
  if (/\b(class|def|self|return|const|let|static)\b/.test(line)) return "#ff7b72";
  if (/\.(place|createLayer|create_layer|build)\b/.test(line)) return "#79c0ff";
...
export default function App() {
  ...
  const [gridWInput, setGridWInput]     = useState(40);
  const [gridHInput, setGridHInput]     = useState(28);
  ...
  const tileEntries = [];
  state.layerDefs.filter(l => l.visible).forEach(layer => {
    Object.entries(state.layers[layer.id] || {}).forEach(([key, tileId]) => {
      const [x, y] = key.split(",").map(Number);
      const tile = allTiles.find(t => t.id === tileId);
      if (tile) tileEntries.push({ x, y, tile, layer, uid: `${layer.id}_${key}` });
    });
  });

  const S = { // shared micro-styles
    btn: (active, col="#388bfd") => ({
      background: active ? "#21262d" : "transparent",
      border: `1px solid ${active ? col : "transparent"}`,
      borderRadius: 5, color: active ? col : "#8b949e",
      padding: "4px 11px", cursor: "pointer", fontSize: 12, fontFamily: "inherit",
    }),
    smallBtn: (col="#8b949e") => ({
      background: "#21262d", border: "none", borderRadius: 3,
      color: col, padding: "4px 6px", cursor: "pointer", fontSize: 10, fontFamily: "inherit",
    }),
    input: (w="100%") => ({
      width: w, background: "#161b22", border: "1px solid #30363d", borderRadius: 4,
  ...
  return (
    <div style={{ height:"100vh", background:"#060a0f", color:"#c9d1d9",
      ...
      {/* ══ TOP BAR ══ */}
      <div style={{ height:44, borderBottom:"1px solid #21262d", background:"#0d1117",
        display:"flex", alignItems:"center", padding:"0 12px", gap:8, flexShrink:0 }}>
        <span style={{ color:"#388bfd", fontWeight:900, fontSize:15, letterSpacing:2 }}>⬡ GAMEFORGE</span>
        <span style={{ color:"#21262d" }}>|</span>
        <input value={worldName} onChange={e=>setWorldName(e.target.value)}
          style={{ ...S.input("140px"), padding:"3px 8px", fontSize:12 }} />
        <div style={{ width:1, height:18, background:"#21262d", margin:"0 2px" }} />
        <button style={S.btn(tool==="paint")} onClick={()=>setTool("paint")}>✏️ Paint</button>
        <button style={S.btn(tool==="erase","#f78166")} onClick={()=>setTool("erase")}>🗑️ Erase</button>
        <button style={{ ...S.btn(false), color:"#8b949e" }} onClick={()=>dispatch({type:"UNDO"})}>↩ Undo</button>
        <div style={{ flex:1 }} />
        <span style={{ fontSize:11, color:"#3d444d" }}>{Math.round(zoom*100)}%</span>
        <button style={{ ...S.btn(false), border:"1px solid #30363d", color:"#8b949e" }}
          onClick={()=>{ setZoom(1); setPan({x:20,y:20}); }}>Reset View</button>
        <button style={{ ...S.btn(showCode,"#388bfd"), border:"1px solid "+(showCode?"#388bfd":"#30363d") }}
          onClick={()=>setShowCode(s=>!s)}>{showCode?"◀ Code":"Code ▶"}</button>
      </div>

      <div style={{ flex:1, display:"flex", overflow:"hidden" }}>

        {/* ══ LEFT SIDEBAR ══ */}
        <div style={{ width:224, background:"#0d1117", borderRight:"1px solid #21262d",
          display:"flex", flexDirection:"column", flexShrink:0, overflow:"hidden" }}>

          {/* Tabs */}
          <div style={{ display:"flex", borderBottom:"1px solid #21262d", flexShrink:0 }}>
            {[["tiles","🧩","Tiles"],["layers","📚","Layers"],["world","⚙️","World"]].map(([id,ic,lb])=>(
              <button key={id} onClick={()=>setLeftTab(id)}
                style={{ flex:1, background:leftTab===id?"#161b22":"transparent", border:"none",
                  borderBottom:`2px solid ${leftTab===id?"#388bfd":"transparent"}`,
                  color:leftTab===id?"#58a6ff":"#8b949e",
                  padding:"7px 0 5px", cursor:"pointer", fontSize:10, fontFamily:"inherit" }}>
                {ic} {lb}
              </button>
            ))}
          </div>
          ...
          {leftTab === "tiles" && (
            <div style={{ flex:1, overflow:"hidden", display:"flex", flexDirection:"column" }}>
              ...
              {!search && (
                <div style={{ display:"flex", flexWrap:"wrap", gap:3, padding:"4px 8px 5px",
                  borderBottom:"1px solid #21262d", flexShrink:0 }}>
                  {Object.keys(TILE_CATEGORIES).map(cat=>(
                    <button key={cat} onClick={()=>setCategory(cat)}
                      style={{ background:category===cat?"#21262d":"transparent",
                        border:`1px solid ${category===cat?"#388bfd":"#21262d"}`,
                        borderRadius:4, color:category===cat?"#58a6ff":"#8b949e",
                        padding:"2px 5px", cursor:"pointer", fontSize:9, fontFamily:"inherit" }}>
                      {cat.split(" ")[0]}
                    </button>
                  ))}
                </div>
              ...
              <div style={{ flex:1, overflowY:"auto", padding:"6px 7px",
                ...
                {filteredTiles.map(tile=>(
                  <button key={tile.id} title={tile.label}
                    ...
                    <span style={{ fontSize:19, lineHeight:1 }}>{tile.icon}</span>
                    <span style={{ fontSize:8, color:activeTile===tile.id?"#58a6ff":"#8b949e",
                      textAlign:"center", overflow:"hidden", width:"100%",
                      textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{tile.label}</span>
                  </button>
                ))}
                {!search && state.customTiles.map(tile=>(
                  <button key={tile.id} title={tile.label}
                    ...
                    <span onMouseDown={e=>{ e.stopPropagation(); dispatch({type:"DEL_CUSTOM",id:tile.id}); }}
                      style={{ position:"absolute",top:2,right:3,fontSize:9,color:"#8b949e",cursor:"pointer" }}>✕</span>
                  </button>
                ))}
              </div>
              {/* Custom tile */}
                    ...
                    <div style={{ display:"flex", gap:4 }}>
                      <input value={cIcon} onChange={e=>setCIcon(e.target.value)}
                        style={{ width:34, background:"#161b22", border:"1px solid #30363d", borderRadius:4,
                          color:"#e6edf3", padding:"3px 2px", fontFamily:"inherit",
                          fontSize:17, textAlign:"center", outline:"none" }} />
                      <input value={cLabel} onChange={e=>setCLabel(e.target.value)} placeholder="Label"
                        style={{ ...S.input(), flex:1, width:"auto" }} />
                    </div>
                    <input value={cCode} onChange={e=>setCCode(e.target.value)} placeholder="CODE_NAME"
                      style={S.input()} />
                    <div style={{ display:"flex", gap:4 }}>
                      <button onClick={()=>{
                      ...
                      }} style={{ flex:1, background:"#196c2e", border:"none", borderRadius:4,
                        color:"#3fb950", padding:"5px", cursor:"pointer", fontSize:11, fontFamily:"inherit" }}>✓ Add</button>
                      <button onClick={()=>setShowCustom(false)}
                        style={{ background:"#21262d", border:"none", borderRadius:4,
                          color:"#8b949e", padding:"5px 8px", cursor:"pointer", fontSize:11, fontFamily:"inherit" }}>✕</button>
                    </div>
                  </div>
                )}
              </div>
                  ...
                  return (
                    <div key={layer.id} onClick={()=>setActiveLayer(layer.id)}
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:7 }}>
                        <span style={{ width:9,height:9,borderRadius:2,background:layer.color,flexShrink:0 }} />
                        <span style={{ flex:1, fontSize:12, color:isActive?"#e6edf3":"#8b949e",
                          overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{layer.name}</span>
                        <span style={{ fontSize:10,color:"#3d444d" }}>{Object.keys(state.layers[layer.id]||{}).length}</span>
                      </div>
                      {isActive && (
                        <div style={{ marginTop:9, display:"flex", flexDirection:"column", gap:6 }}>
                          <div style={{ display:"flex", gap:4 }}>
                            <button onClick={e=>{e.stopPropagation();dispatch({type:"TOGGLE_VISIBLE",id:layer.id});}}
                              style={{ ...S.smallBtn(layer.visible?"#3fb950":"#8b949e"), flex:1 }}>
                              {layer.visible?"👁 Visible":"🙈 Hidden"}</button>
                            <button onClick={e=>{e.stopPropagation();dispatch({type:"TOGGLE_LOCK",id:layer.id});}}
                              style={{ ...S.smallBtn(layer.locked?"#f78166":"#8b949e"), flex:1 }}>
                              {layer.locked?"🔒 Locked":"🔓 Free"}</button>
                          </div>
                          <div style={{ display:"flex", alignItems:"center", gap:5 }}>
                            ...
                            <span style={{ fontSize:9,color:"#8b949e",width:26,textAlign:"right" }}>
                              {Math.round((layer.opacity??1)*100)}%</span>
                          </div>
                          <div style={{ display:"flex", gap:3 }}>
                            <button onClick={e=>{e.stopPropagation();dispatch({type:"REORDER",from:realIdx,to:Math.max(0,realIdx-1)});}}
                              style={S.smallBtn()}>↑</button>
                            <button onClick={e=>{e.stopPropagation();dispatch({type:"REORDER",from:realIdx,to:Math.min(state.layerDefs.length-1,realIdx+1)});}}
                              style={S.smallBtn()}>↓</button>
                            <button onClick={e=>{e.stopPropagation();if(window.confirm(`Clear "${layer.name}"?`))dispatch({type:"CLEAR_LAYER",id:layer.id});}}
                              style={{ ...S.smallBtn(), flex:1 }}>Clear</button>
                            {state.layerDefs.length > 1 &&
                              <button onClick={e=>{e.stopPropagation();if(window.confirm(`Delete "${layer.name}"?`)){dispatch({type:"REMOVE_LAYER",id:layer.id});setActiveLayer(state.layerDefs.find(l=>l.id!==layer.id)?.id||"");}}}
                                style={{ ...S.smallBtn("#f78166"), background:"#3d1a1a" }}>🗑</button>}
                          </div>
                        </div>
                      )}
                    </div>
                ...
                {!showAddLayer ? (
                  ...
                  <div style={{ display:"flex", gap:5 }}>
                    <input value={newLayerName} onChange={e=>setNewLayerName(e.target.value)}
                      placeholder="Layer name" autoFocus style={{ ...S.input(), flex:1, width:"auto" }} />
                    <button onClick={()=>{
                    ...
                    }} style={{ background:"#196c2e",border:"none",borderRadius:4,color:"#3fb950",padding:"5px 9px",cursor:"pointer",fontFamily:"inherit",fontSize:11 }}>✓</button>
          ...
          {leftTab === "world" && (
            <div style={{ flex:1, overflowY:"auto", padding:"12px 10px" }}>
              ...
              <div style={{ display:"flex", gap:6, marginBottom:7 }}>
                {[["Width",gridWInput,setGridWInput],["Height",gridHInput,setGridHInput]].map(([label,val,set])=>(
              ...
              <button onClick={()=>dispatch({type:"SET_GRID",w:gridWInput,h:gridHInput})}
              ...
              {[["Grid",`${state.gridW}×${state.gridH}`],["Layers",state.layerDefs.length],
                ["Objects",totalObjects],["Custom Tiles",state.customTiles.length],["History",state.history.length]
              ].map(([k,v])=>(
              ...
              <div style={{ fontSize:11, color:"#8b949e", lineHeight:2.1 }}>
        ...
        <div ref={canvasRef}
          ...
          <div style={{ position:"absolute", left:pan.x, top:pan.y }}>
            ...
            <svg style={{ position:"absolute",top:0,left:0,pointerEvents:"none" }}
              ...
              <rect width="100%" height="100%" fill="none" stroke="#388bfd" strokeWidth={1.5} opacity={0.2}/>
            </svg>

            {/* Tiles */}
            {tileEntries.map(({ x, y, tile, layer, uid }) => (
              <div key={uid} style={{
                position:"absolute",
                left: x*TILE_SIZE*zoom, top: y*TILE_SIZE*zoom,
                width: TILE_SIZE*zoom, height: TILE_SIZE*zoom,
                background: tile.color, opacity: layer.opacity??1,
                display:"flex", alignItems:"center", justifyContent:"center",
                fontSize: Math.max(9, TILE_SIZE*zoom*0.58), lineHeight:1, pointerEvents:"none",
              }}>{tile.icon}</div>
            ))}
            ...
            {hovered && hovered.x>=0 && hovered.x<state.gridW && hovered.y>=0 && hovered.y<state.gridH && (
              <div style={{
                ...
                border:`${Math.max(1,zoom*0.8)}px solid ${tool==="erase"?"#f78166":"#388bfd"}`,
                display:"flex", alignItems:"center", justifyContent:"center",
                fontSize: Math.max(9, TILE_SIZE*zoom*0.58),
                opacity:0.7, pointerEvents:"none", boxSizing:"border-box",
              }}>{tool!=="erase" && activeTileDef?.icon}</div>
            )}
          </div>

          {/* Status bar */}
          <div style={{ position:"absolute", bottom:0, left:0, right:0, height:25,
            background:"rgba(13,17,23,0.92)", borderTop:"1px solid #21262d",
            display:"flex", alignItems:"center", gap:18, padding:"0 14px",
            fontSize:11, color:"#3d444d" }}>
            <span>Grid <span style={{ color:"#8b949e" }}>{state.gridW}×{state.gridH}</span></span>
            {hovered && <span>Cell <span style={{ color:"#58a6ff" }}>[{hovered.x},{hovered.y}]</span></span>}
        ...
        {/* ══ CODE PANEL ══ */}
        {showCode && (
          <div style={{ width:290, borderLeft:"1px solid #21262d", background:"#0d1117",
              ...
              {["js","py","json"].map(l=>(
                <button key={l} onClick={()=>setCodelang(l)}
                  style={{ background:codelang===l?"#21262d":"transparent",
                    ...
                    padding:"2px 7px", cursor:"pointer", fontSize:10, fontFamily:"inherit" }}>{l}</button>
              ))}
              ...
              <pre style={{ margin:0, fontSize:11, lineHeight:1.7, whiteSpace:"pre-wrap", wordBreak:"break-all" }}>
                {code.split("\n").map((line,i)=>(
                  <div key={i} style={{ display:"flex", gap:10 }}>
                    <span style={{ color:"#3d444d",minWidth:18,textAlign:"right",flexShrink:0,fontSize:9,paddingTop:1 }}>{i+1}</span>
